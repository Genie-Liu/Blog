...
* 参数(parameter): 每次调参所记录的参数，记录为{key: value}的形式， 每次run要求key都一样
...
展开metrics, params, tags目录，我们看到之前在脚本中记录的参数，其中之前的每一个key-value对在真实数据中，key为文件名，而value则为文件内容。其中metric的文件内容里面，还增加了时间戳
